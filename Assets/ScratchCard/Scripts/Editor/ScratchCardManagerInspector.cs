				...
				if (card.objectReferenceValue != null)
				{
					...
					if (cardManager.Card.RenderTexture != null)
					{
						...
						if (Application.isPlaying)
						{
							...
							var halfWidth  = (EditorGUIUtility.currentViewWidth - padding) * 0.5f;
							EditorGUILayout.BeginHorizontal();
							if (GUILayout.Button("Clear", GUILayout.Width(halfWidth)))
							{
								cardManager.ClearScratchCard();
							}
							if (GUILayout.Button("Fill", GUILayout.Width(halfWidth - paddingRight)))
							{
