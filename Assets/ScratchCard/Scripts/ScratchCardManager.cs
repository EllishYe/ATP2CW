...
namespace ScratchCardAsset
{
	...
	public class ScratchCardManager : MonoBehaviour
	{
		...
		public bool SaveProgressTexture(string fileName)
		{
			RenderTexture temp = null;
			Texture2D resultTexture = null;
			try
			{
				var renderTexture = Card.RenderTexture;
				...
				Graphics.Blit(Card.RenderTexture, temp);

				resultTexture = new Texture2D(renderTexture.width, renderTexture.height, TextureFormat.R8, false);
				...
				var filePath = Path.Combine(Application.persistentDataPath, $"{fileName}.png");
		...
		public bool LoadProgressTexture(string fileName)
		{
			...
			try
			{
				...
				tempTexture = new Texture2D(1, 1, TextureFormat.R8, false);
				...
				{
					Debug.LogError("Target RenderTexture is null.");
					return false;
				}

				Graphics.Blit(tempTexture, Card.RenderTexture);
				Progress.UpdateProgress();
				return true;
			}
			catch (Exception ex)
		...
		public bool DeleteProgressTexture(string fileName)
		{
			try
			{
				var filePath = Path.Combine(Application.persistentDataPath, $"{fileName}.png");
				...
				{
					Debug.LogWarning($"File not found when attempting to delete: {filePath}");
					return false;
				}
        
				File.Delete(filePath);
				Debug.Log($"Successfully deleted progress file: {filePath}");
				return true;
			}
			catch (Exception ex)
			{
				Debug.LogError($"Error deleting progress file: {ex.Message}");
				return false;
			}
		}
		
		public bool IsProgressTextureExists(string fileName)
		{
			try
			{
				var filePath = Path.Combine(Application.persistentDataPath, $"{fileName}.png");
				...
				if (!exists)
				{
					Debug.Log($"Progress file does not exist: {filePath}");
				}
			...
			{
				Debug.LogError($"Error checking if progress file exists: {ex.Message}");
				return false;
			}
		}
